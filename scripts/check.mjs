...
if (articles.length !== 9) failures.push(`content/articles.json: expected 9 articles, found ${articles.length}`);
...
if (articles.filter((article) => article.product === "journal").length !== 7) failures.push("content/articles.json: expected exactly 7 Journal articles");

for (const article of articles) {
  const minimumSources = article.type === "Literary essay" ? 2 : 6;
  if (article.sources.length < minimumSources) failures.push(`content/articles.json: ${article.slug} has fewer than ${minimumSources} sources`);
...
for (const required of ["CNAME", ".nojekyll", "feed.xml", "sitemap.xml", "robots.txt", "404.html", "projects/index.html", "assets/favicon.svg", "assets/images/bitcoin-donation-qr.svg"]) {
