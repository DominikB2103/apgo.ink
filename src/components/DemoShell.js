...
export function DemoShell(demo){
  return html`${Nav({active:'examples'})}<main class="demo-shell demo-page">
    <section class="demo-hero"><div class="container demo-hero-grid">
      <div class="reveal"><span class="demo-label">${demo.type} website example</span><h1 class="demo-title">${demo.name}</h1><p class="demo-lead">${demo.lead}</p><div class="demo-actions"><a class="btn demo-primary" href="#book">${demo.cta}<span>→</span></a><a class="btn demo-secondary" href="#services">${demo.secondary}</a></div></div>
    ...
    <section class="demo-section" id="book"><div class="container"><div class="demo-footer-cta reveal"><h2>Like this direction?</h2><p>This example is designed to show what a finished industry-specific website can feel like before a business commits.</p><div class="demo-actions"><a class="btn btn-primary" href="/">Back to APTO.INK</a><a class="btn btn-ghost" href="/demos/">View more examples</a></div></div></div></section>
  </main>${Footer()}`;
}
